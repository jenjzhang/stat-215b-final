...
\begin{columns}[T]
  \begin{column}{0.52\textwidth}
    ...
    Llama: \textbf{2.0\%} — more diffuse miscalibration.\\[0.3em]
    Domain variance $= 0$: only 4 groups; unidentifiable.
  \end{column}
...
\note{
  ...
  estimate domain-level variance reliably. We treat domain as a descriptive label.''
}
...
\begin{frame}{Cross-model agreement: miscalibration is task-driven}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \includegraphics[width=\textwidth]{figures/cross_model_ece.pdf}
    \end{column}
    \begin{column}{0.48\textwidth}
      \small
      ...
      \vspace{0.5em}
      Same subjects are hardest to calibrate on regardless of model family, scale,
...
\note{
  ...
  rankings are highly consistent, and 57/57 subjects are flagged by both.''

  The key takeaway: ``This is not a GPT problem or a Llama problem. It's a knowledge
  domain problem. The subjects that are hard to calibrate on are hard regardless of
  what model you use. That means recalibration needs to happen at the subject level,
  not the model level.''
...
\note{
  ...
  Spearman above 0.984 for both models — the top-5 miscalibrated subjects are stable.''
...
\begin{enumerate}
  ...
  \item \textbf{Miscalibration is task-driven, not model-specific.} Spearman
    $\rho = 0.846$, 57/57 subjects flagged by both models. The same domains are
    hardest to calibrate on regardless of model.
