...
larger models (Spearman $\rho \in [0.79, 0.94]$, all 15 pairs); the smallest
...
the entropy coefficient grows log-linearly with scale ($-0.004 \to +0.006
\to +0.029 \to +0.040$), and between-subject ICC grows substantially from 1.5B
to 3B (0.017 $\to$ 0.051) and remains elevated at the larger scales
...
The full pipeline is run on each of the seven models. Subjects flagged as
...
training. We compare subject-level ECE rankings across all 21 model pairs using
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\textwidth]{figures/coef_plot_7m.pdf}
...
grows log-linearly across all four scale points: $-0.004 \to +0.006 \to +0.029
...
four matched-precision checkpoints (1.5B, 3B, 7B, 14B): both ICC and the entropy
coefficient grow log-linearly with scale, while the intercept peaks at 3B
and dips at 7B and 14B as accuracy gains partially offset the more concentrated
overconfidence pattern.

\begin{figure}[h]
  \centering
  \includegraphics[width=\textwidth]{figures/qwen_scaling_7m.pdf}
  \caption{Within-Qwen scaling on four matched-precision checkpoints
  (1.5B fp16, 3B 4-bit, 7B 4-bit, 14B 4-bit). Both ICC$_\text{subject}$ and the entropy
  coefficient grow log-linearly with scale; the mean-overconfidence intercept
  is non-monotonic, peaking at 3B and dipping at 7B and 14B as accuracy gains
...
log-linearly across the 1.5B/3B/7B/14B scaling sweep---an observational pattern
