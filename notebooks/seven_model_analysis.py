"""
Seven-model analysis (extends six_model_analysis.py with Qwen-2.5-3B-Instruct).

Main story: GPT-4o-mini, Llama-3-8B-Instruct, Qwen-{0.5B, 1.5B fp16, 3B, 7B, 14B}-Instruct.
...
QWEN_SCALE = ["qwen_0_5b", "qwen_1_5b_fp16", "qwen_3b", "qwen_7b", "qwen_14b"]
QWEN_SCALE_MAIN = ["qwen_1_5b_fp16", "qwen_3b", "qwen_7b", "qwen_14b"]  # excludes 0.5B
...
for N in [5, 10, 15]:
    ...
    print(f"  Top-{N} ∩ all 7 main models: {len(common)} subjects -> {sorted(common)}")
...
for N in [5, 10, 15]:
    ...
    common = set.intersection(*sets)
    print(f"  Bottom-{N} ∩ all 7 main models: {len(common)} subjects -> {sorted(common)}")
...
fixed_effects = {
    ...
    "qwen_3b":        {"intercept": ( 0.311, "<0.001"), "word_count": ( 0.010, 0.127),  "max_choice_len": (-0.008, 0.084), "has_negation": (-0.014, 0.211), "entropy": ( 0.006, 0.151)},
...
icc = {
    ...
    "qwen_3b":        {"icc_subject": 0.051, "var_subject": 0.011, "var_residual": 0.203},
...
fdr_counts = {
    ...
    "qwen_3b":        {0.01: 57, 0.05: 57, 0.10: 57},
...
with open(PROC / "seven_model_summary.json", "w") as f:
    json.dump(out, f, indent=2, default=str)
print(f"\nSaved summary to {PROC / 'seven_model_summary.json'}")
