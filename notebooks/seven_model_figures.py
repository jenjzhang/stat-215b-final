"""
Generate all figures for the seven-model report revision.
Outputs to report/figures/ with `_7m` suffix.
"""
import json
import numpy as np
import pandas as pd
...
def figure_top_or_bottom(model, kind, n_show=5):
    ...
    fig.tight_layout()
    out = FIG / f"{kind}5_{label.replace(' ', '_')}_{model}_7m.pdf"
...
figure_corr_heatmap(MAIN_MODELS, "7m")
...
def figure_subject_mean_gap():
    ...
    fig.suptitle("Subject-level mean calibration gap (red = FDR-rejected at α=0.05)",
                 fontsize=11, y=0.99)
    fig.tight_layout()
    out = FIG / "subject_mean_gap_7m.pdf"
    fig.savefig(out, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved {out}")
...
def figure_qwen_scaling():
    ...
    scale_data = pd.DataFrame({
        ...
        "intercept":  [0.179, 0.311, 0.233, 0.210],
        "intercept_se": [0.009, 0.015, 0.014, 0.014],
        "icc":        [0.017, 0.051, 0.049, 0.059],
        "entropy":    [-0.004, 0.006, 0.029, 0.040],
        "entropy_se": [0.004, 0.004, 0.004, 0.003],
        "accuracy":   [0.571, 0.601, 0.685, 0.757],
    })
